// Package epoll lengdanran 2024/3/29 14:56
//go:build darwin
// +build darwin
...
func (socket BsdSocket) Read(bytes []byte) (int, error) {
	if len(bytes) == 0 {
		return 0, nil
	}
	numBytesRead, err :=
...
func (socket BsdSocket) Write(bytes []byte) (int, error) {
	numBytesWritten, err :=
...
func (socket *BsdSocket) Close() error {
	if socket.IsOpen {
...
func (socket *BsdSocket) String() string {
	return strconv.Itoa(socket.FileDescriptor)
}

func FromFileDescriptor(fileDescriptor int) *BsdSocket {
	return &BsdSocket{FileDescriptor: fileDescriptor, IsOpen: true}
}

func Listen(ip string, port int) (*BsdSocket, error) {
	socket := &BsdSocket{}
